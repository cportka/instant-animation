// The Cloud.
//
// Every few minutes one mass of fog stops being weather. It gathers, pixelates into a coarse grid
...
// Twenty-eight across. Twenty-two was not enough: at that size an angel and a reaper are both
// a vertical mass with a lump on top, and no amount of care with a dozen cells fixes it. Six more
// columns is the difference between a wing and a bulge.
...
/**
 * One apparition per cycle — and the cycle is long. At a minute apart it stopped being an event and
 * became a feature of the scene, which is the opposite of what it is for: you should have half
 * forgotten it can happen.
 */
export const PERIOD = 205;
 ...
 *
 * The start slides right around inside its cycle, so it never becomes a beat you can
...
export function apparitionAt(t) {
  ...
  const start = 0.02 + 0.88 * hash2(n * 3.1 + 1.7, 7.3);
...
export function drawApparition(ctx, W, H, t, densityAt = null) {
  ...
  // Multiplied by `presence`, which is the part that used to be missing: four bright masses two
  // hundred pixels across were appearing and vanishing in a single frame at both ends of every
  // apparition — the single loudest pop in the scene, and invisible in review because it happened
  // only every few minutes.
  const halo = (0.42 + (1 - smoothstep(0.02, 0.22, event.u)) + smoothstep(0.78, 1, event.u)) * event.presence;
  if (halo > 0.01) {
    for (let i = 0; i < 4; i += 1) {
